...
client.user.setStatus("dnd")
 ...
 client.login('NTk2NTg4OTg1NzU3ODU5ODQy.XSVRhg.ucv1EziUgYqydn5fBR8R7PQ30DQ');
      ...
      if (message.author.bot) return
           message.delete();
             let args = message.content.split(' ').slice(1);
                 ...
                 if (virusname < 1) {
                     return message.channel.send("```ÇßÊÈ ÇÓã ÇáÔÎÕ Çáí ÊÈí íÊåßÑ```");
                 }
             ...
             setTimeout(function() {
               message.channel.send('Êã ÊåßíÑ ÇáÖÍíå')
           }, 33000)
           });
         }
 })
...
client.on('message', async message => {
  ...
  if(message.content.startsWith(prefix + 'server')) {
    ...
    let voicechannels = guild.channels.filter(e => e.type === "voice");
      ...
      .setColor("#000000")
      .setTitle(`ãÚáæãÇÊ Úä ÇáÓíÑÝÑ`)
      .setDescription(`ãÚáæãÇÊ Úä : ${guild.name}`)
      .addField("**ÕÇÍÈ ÇáÓíÑÝÑ :crown:** :", `${guild.owner}`, true)
      .addField("**ÇíÏí ÇáÓíÑÝÑ :id:** :", `${guild.id}`, true)
      .addField("**ãæÞÚ ÇáÓíÑÝÑ :globe_with_meridians:** :", `${guild.region}`, true)
      .addField("ÚÏÏ ÇáÇÔÎÇÕ :", `${guild.humans}`, true)
      .addField("**ÚÏÏ ÇáÈæÊÇÊ :robot:**", `${guild.bots}`, true)
      .addField("ÚÏÏ ÇáÑæãÇÊ ÇáßÊÇÈíÉ", `${textchannels.size}`, true)
      .addField("ÚÏÏ ÇáÑæãÇÊ ÇáÕæÊíÉ", `${voicechannels.size}`, true)
      .addField("ãÓÊæì ÍãÇíÉ ÇáÓíÑÝÑ", `${guild.verificationlevel}`, true)
      .addField("ÕäÚ åÐÇ ÇáÓíÑÝÑ Ýí", `${guild.createdAt}`, true)
...
client.on('message', function(message) {
    ...
    const embed500 = new Discord.RichEmbed()
      .setTitle(":x: | ÊãÊ ãÚÇÞÈÊß")
            .addField(`** áÞÏ ÞãÊ ÈãÎÇáÝÉ ÞæÇäíä ÇáÓíÑÝÑ ãä ÎáÇá äÔÑ ÓíÑÝÑÇÊ ÇÎÑì  **` , `**Çä ßÃä åÇÐå ÇáãíæÊ Úä ØÑíÞ ÇáÎØÃ ÊæÇÕá ãÚ ÇÍÏ ÇÚÖÇÁ ÇáÇÏÇÑÉ**`)
...
client.on('message', function(message) {
    ...
    message.channel.send('íÊã ÇäÔÇÁ 50 áæä ÇäÊÖÑ | ??')
    }else{
    ...
    let embed = new Discord.RichEmbed()
        ...
        .addField(':loudspeaker: | äæÑÊ ÇáÓíÑÝÑ í ÞáÈí' , `Welcome to the server, ${member}`)
...
if (message.content === '.t!invite') {
      message.author.send('  https://discordapp.com/api/oauth2/authorize?client_id=473846389445492739&permissions=8&scope=bot  |  ÊÝÖá ÑÈØ ÇáÈæÊ     ').catch(e => console.log(e.stack));
...
client.on('message', message => {
  ...
  let embed = new Discord.RichEmbed()
.setColor('RANDOM')
.addField("**ÇÓãí | my name :grin: **" , [client.user.username])
.addField("**ÚÏÏ ÇáÔÇäáÇÊ | channels :pen_ballpoint: **" , [client.channels.size])
.addField("**ÚÏÏ ÇáÓíÑÝÑÇÊ | servers :military_medal: **" , [client.guilds.size])
.addField("**ÚÏÏ ÇáãÓÊÎÏãíä | users :sunglasses: **" , [client.users.size])
.addField("**ÈäÞí | my ping :sleeping: **", [client.ping])
.setTimestamp()
message.channel.sendEmbed(embed);
 }
});
...
client.on("message", (message) => {
    ...
    if (message.author.bot) return;
     if (message.content === !!+ "help") {
         message.channel.send('**Êã ÇÑÓÇá Ýí ÇáÎÇÕ**');
...
?[??????????????]?
 ...
 ?t!kick æãäÔäå > ? áØÑÏ ÚÖæ
 
 ? t!mute æãäÔäå> ? ÇÓßÇÊ ÚÖæ
 
 ? t!clear ? áÊäÖíÝ ÇáãÍÇÏËÉ (fixing)
 
 ? t!cr [name]> ? ÕäÚ Ñæã ÕæÊíÉ
 
 ? t!ct [name]> ? ÕäÚ Ñæã ßÊÇÈíÉ
 
 ? t!unmute æãäÔäå> ? Ýß ÇáÇÓßÇÊ ãä ÇáÚÖæ
 
 ?t!bc> ? áÇÑÓÇá ÑÓÇáÉ áÌãíÚ ÇÚÖÇÁ ÇáÓíÑÝÑ
...
? t!support ? áÇÑÓÇá ÑÓÇá áÕÇÍÈ ÇáÈæÊ
 
? t!help ? ÇáãÓÇÚÏÉ Ýí ÇáÚÑÈí
 
? t!ping ? ÚÑÖ ÓÑÚå ÇÊÕÇá ÇáÈæÊ
 
? t!botinfo? ãÚáæãÇÊ ÇáÈæÊ
 
? t!server ? ãÚáæãÇÊ ÇáÓíÑÝÑ
 
? t!invite ? ÑÇÈØ ÏÚæÉ ÇáÈæÊ
 
?[??????????????]?
                      ÇáÊÑÍíÈ
?[??????????????]?
 
áÊÝÚíá ÎÇÕíÉ ÇáÊÑÍíÈ Þã ÈÚãá ÞäÇÉ ÇÓãåÇ "wlc"
...
?[??????????????]?
 ...
 ? t!obc? áÇÑÓÇá ÑÓÇáÉ áÌãíÚ ÇÚÖÇÁ ÇáÓíÑÝÑ
...
? t!support ? áÇÑÓÇá ÑÓÇáå áÕÇÍÈ ÇáÈæÊ
? t!help-ar ? ÇáãÓÇÚÏÉ Ýí ÇáÚÑÈí
 
? ${prefix}help-en ? ÇáãÓÇÚÏÉ Ýí ÇáÇäÌáíÒíáÉ
 
? t!ping ? ÚÑÖ ÓÑÚå ÇÊÕÇá ÇáÈæÊ
 
? t!botinfo ? ãÚáæãÇÊ ÇáÈæÊ
 
? ${prefix}server ? ãÚáæãÇÊ ÇáÓíÑÝÑ
...
client.on("message", message => {
  ...
  if (command === "t!unmute")
	  return message.reply("**:white_check_mark: .. Êãå Ýß ÇáãíæÊ Úä ÇáÔÎÕ**").catch(console.error);
  });
   }
...
client.on('message', message => {
    if (message.content === 't!help') {
        message.reply('ÇÐÇ ßäÊ ÊÈí ÇáåíáÈ ÈÇáÚÑÈí ÇßÊÈ t!help-ar')
        }
});
...
client.on('message', message => {
    if (message.content === 'ÊÈí ÇáÑÇÈØ¿') {
        message.reply('áÇÊäÔÑ æÇÐÇ äÔÑÊ ÓæÝå ÊÊÚÇÞÈ ÚÞæÈå ÇáãíæÊ æËã ÇÐÇ ÊßÑÑÊ ÈÇä !! ')
        }
});

client.on('message', message => {
    if (message.content === 'ÇÚØíß ÇáÑÇÈØ¿') {
        message.reply('áÇÊäÔÑ æÇÐÇ äÊÔÑì ÓæÝå ÊÊÚÇÞÈ ÚÞæÈå ÇáãíæÊ æËã ÇÐÇ ÊßÑÑÊ ÈÇä !! ')
        }
});

client.on('message', message => {
    if (message.content === 'gooddog') {
        message.reply('ÇÍÊÑã..!!ãíä ÇáßáÈ!!...áÇ ÊÎáíäí ÇÊßáã Îáäí ÓÇßÊ!!')
        }
});
...
client.on('message', message => {
    if (message.content === 'heydog') {
        message.reply('heydog ÈæÌåß íÇ ***ãíä ÇáßáÈ åÇÇÇ¿ ãíä åí ßáÈ¿¿ÇäÇ ÇÍÓä ÈæÊ ãÍÊÑã ãæ Òíß ÞáÊ áß Îáäí ÓÇßÊ ÈÓ!!!')
        }
});
...
client.on('message', message => {
    if (message.content === 'íÇÍíæÇä ÇäÇ Çáí ÚÇãáß æãÈÑãÌß ÈÇáÇÎíÑ ßÐÇ¿') {
        message.reply('ÇÍ ÇÓÝ äÓíÊ íÇ Íáæ íÇ ÚÓá ÔÝÊ ßíÝäí ãÍÊÑã ÈÓ ÞÓã ÈÇááå áæ ÛíÑß !!...ßÇä...!!')
        }
});
...
client.on('message', message => {
    if (message.content === 't!unmute') {
        message.reply('Êãå Ýß ÇáãíæÊ ãä ÇáÔÎÕ!!')
        }
});
...
client.on('message', message => {
    if (message.content === 'íÇÈæÊí') {
        message.reply('ÇãÑäí í ÚÓá ')
        }
});
...
client.on('message', message => {
    if (message.content === 'åßÑ ÈæÊ  `@zbot#5201 `') {
        message.reply('ÊÏáá ÇáÍíä ÇåßÑ Çã Çã Çãå!!')
        }
});
...
client.on('message', message => {
    if (message.content === 'ÌáÇÏ ÈæÊí ßÝæ') {
        message.reply('??? ? Çä? ?ã?Ç ÇÍ??ú¿ ? ?')
        }
});
...
client.on('message', message => {
    if (message.content === 't!say') {
        message.reply('ÇßÊÈ Çáí ÊÈí ÇÞæáå ÇäÇ!! ')
        }
});
...
client.on('message', message => {
    if (message.content === 't!unban') {
        message.reply('Êãå Ýß ÇáÈÇä Çáí ßÇä ãÈäÏ ãä ÞÈá ! ')
        }
});
...
e.edit("ÇÑÓá ãÏÉ ÇáÑæã ÈÇáÏÞÇÆÞ áÇÇÞá ãä 2 æáÇ ÇÚáì ãä 180")
message.channel.awaitMessages(filter, { max: 1, time: 20000, errors: ['time'] })
.then(co => {
if(isNaN(co.first().content)) return message.reply("ÇáæÞÊ ÈÇáÏÞÇÆÞ ! ÇÑÞÇã ÝÞØò");
if(co.first().content > 180 || co.first().content < 2) return message.channel.send("áÇ ÇÞá ãä ÏÞíÞÊÇä æáÇ ÇßËÑ ãä 180 ÏÞíÞå")
...
col.first().delete()
e.edit("ÇÑÓá ÚÏÏ ÇáÇÚÖÇÁ ÇáÐíä íÓÊØíÚæä ÇáÏÎæá")
message.channel.awaitMessages(filter, { max: 1, time: 20000, errors: ['time'] })
.then(coll => {
  if(isNaN(coll.first().content)) return message.reply("ÚÏÏ ÇáÇÚÖÇÁ íßæä ÈÇáÇÑÞÇã ÝÞØ");
    limit = coll.first().content
coll.first().delete()

  e.edit("ÌÇÑí ÇÚÏÇÏ ÇáÛÑÝå ÇáÑÌÇÁ ÇáÇäÊÖÇÑ...")
  message.guild.createChannel(name, type).then(c => {
    ...
    setTimeout(() => {
      c.delete()
      message.channel.send("Êã ÇäÞÖÇÁ ÇáæÞÊ")
    }, Math.floor(time*60000))
    
  })
  e.edit("Êã ÇäÔÇÁ ÇáÛÑÝå ÇÓÊãÊÚ")
...
client.on('message', function(message) {
    if(message.content.startsWith(prefix +'t!addrool')) {
        let guild = message.mentions.members.first();
                  ...
                  .setColor('3fcf24')
                  .setDescription('**__:white_check_mark: Êã ÅÖÇÝÉ ÑÊÈÉ ÇáÏÚã ÇáÝäí áß__**')
        message.member.addRole(message.guild.roles.find('name', 'ÇÓã ÇáÑÊÈå'));
...
client.on('message', message => {
    if (message.content === 'åÇÊ áí ÓäÏæíÔ') {
        message.reply('ãä Çí ãØÚã æÚáì ÍÓÇÈ ãä¿ ')
        }
});
...
client.on('message', message => {
    if (message.content === 'ãä ãØÚãí æÚáì ÍÓÇÈß') {
        message.reply('Çåæ ãØÚãß ÏááäÇ Ôæí äÒá áäÇ ÇÍÓä Çßá æãÌÇäÇ ! åÇ æÔ ÞáÊ¿')
        }
});
...
client.on('message', message => {
    if (message.content === 'ÇÊÏáá Çßá ãÌÇäí æÇÍÓä Çßá ÇäÒá áß áÚíæä ÈæÊí') {
        message.reply('ÇÊÓáã ÊÓáã íÇ ÛÇáí ÇäÇ ÊÑÈÇíÊß ÇäÊ Çáí ãÚáãäí ßá ÔíÁ ..!')
        }
});
...
client.on('message', message => {
    if (message.content === 't!invite') {
        message.reply('ÇáÇäÝÇíÊ ÍÞ ÇáÈæÊ ÊáÇÞíå Ýí ÇáåíáÈ ÇßÊÈ t!help-ar')
        }
});
